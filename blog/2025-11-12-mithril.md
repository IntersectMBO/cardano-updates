...
This week, the Mithril team released the `2543.1` hotfix distribution which fixes an issue with the previous release leading some Mithril signers to stop signing until the end of the current epoch when being restarted.

The team has almost finalized the work on the first phase of decentralizing configuration parameters. They have also continued implementing a simple aggregator discovery mechanism. Additionally, they kept progressing on the SNARK-friendly STM library by designing its architecture, implementing the Schnorr signature scheme, refactoring the error handling, and experimenting with a Jubjub curve implementation in the BLST library.

Finally, the team worked on the refactoring of an interface used to interact with the aggregator in the signer, and made improvements to the protocol security page.
